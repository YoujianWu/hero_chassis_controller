...
HeroChassisController::~HeroChassisController() { sub_command_.shutdown(); }
...
bool HeroChassisController::init(
  ...
  controller_nh.getParam("Wheel_R", Wheel_R);
  controller_nh.getParam("Odom_mode", Odom_mode);
  ...
  controller_state_publisher_ = std::make_unique<
      realtime_tools::RealtimePublisher<control_msgs::JointControllerState>>(
      root_nh, "state", 1);

  // initialize the subscriber
  sub_command_ = root_nh.subscribe<geometry_msgs::Twist>(
...
void HeroChassisController::return_state(
    const geometry_msgs::TwistConstPtr &state_msg) {
...
void HeroChassisController::vel_mode(bool &Mode) {
